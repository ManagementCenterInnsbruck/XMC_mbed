...
void serial_init(serial_t *obj, PinName tx, PinName rx)
{
	...
	else
	{
		...
		gpio_t gpio;
		gpio_init(&gpio, obj_s->rx_pin);
		XMC_GPIO_Init(gpio.port, gpio.pin, &rx_pin_config);
		...
		gpio_init(&gpio, obj_s->tx_pin);
		XMC_GPIO_Init(gpio.port, gpio.pin, &tx_pin_config);
	}
